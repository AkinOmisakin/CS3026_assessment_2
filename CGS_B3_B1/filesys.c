...
MyFILE * myfopen ( const char * filename, const char * mode )
{
   ...
   if (strcmp("w", mode) == 0)
   {
      ...
      virtualDisk[3].dir.entrylist[virtualDisk[3].dir.nextEntry] = file_ptr->buffer.dir.entrylist[0]; // set root dir nextEntry to entry above

      //file_ptr->buffer.dir.nextEntry = virtualDisk[3].dir.nextEntry; // save current entry location

      strncpy(virtualDisk[3].dir.entrylist[virtualDisk[3].dir.nextEntry].name, filename, MAXNAME); // set name again
...
void myfclose ( MyFILE * stream )
{
   ...
   if (dirIndex >=0)
   {
      ...
      virtualDisk[3].dir.entrylist[dirIndex].filelength = (Byte) stream->buffer.dir.entrylist[0].filelength+stream->pos;
...
void myfputc ( int b, MyFILE * stream ) 
{
   ...
   {
      stream->buffer.dir.entrylist[0].filelength += (stream->pos);
...
void mymkdir ( const char * path )
{
   ...
   while ((token = strtok_r(pathCopy, "/", &rest)) != NULL)
   {
      ...
      {
         // if index found
         if (strcmp(currentParent->entrylist[i].name, token) == 0 )
         {
            printf("Directory %s already exists\n", token);
            // then traverse parent to firstlevel
            currentParent = &currentParent->entrylist[i];
         ...
         else /* if the directory (token) is not found in parent cell*/
         {
            ...
            currentParent->entrylist[dirIndex].firstblock = ENDOFCHAIN;
